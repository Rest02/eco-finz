...
### [] Fase 2: Migración y Estandarización de Auth

Movemos la lógica de autenticación dispersa a su nuevo hogar modular.

*   **[] Tarea 2.1: Migración de Tipos y Servicios.**
    *   Mover `src/types/auth.ts` -> `src/features/auth/types/auth.ts`.
    *   Mover `src/services/authService.ts` -> `src/features/auth/services/authService.ts`.
    *   Actualizar importaciones en el servicio (si referenciaba tipos antiguos).

*   **[] Tarea 2.2: Migración de Componentes.**
    *   Mover componentes de `src/components/auth/*` -> `src/features/auth/components/*`.
    *   (Opcional) Si existe un Contexto de Auth, moverlo a `src/features/auth/context` o `hooks`.

*   **[] Tarea 2.3: Actualización de Referencias.**
    *   Actualizar todas las importaciones en `src/app/(auth)/*` y `src/middleware.ts` (si aplica) para apuntar a las nuevas rutas en `src/features/auth`.
    *   Eliminar las carpetas antiguas `src/components/auth`, `src/services` (si queda vacía) y `src/types` (si queda vacía).
