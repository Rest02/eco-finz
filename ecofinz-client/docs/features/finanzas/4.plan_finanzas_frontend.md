...
#### **[] Fase 6: Gestión de Presupuestos (CRUD)**

[] Esta fase implementará la funcionalidad que permite a los usuarios definir y gestionar sus metas de gasto mensuales por categoría.

*   **[] Tarea 6.1: Extender `financeService` para Presupuestos.**
    *   [] En `src/finance/services/financeService.ts`, añadir las funciones para interactuar con los endpoints de presupuestos: `getBudgets(year, month)`, `createBudget`, `updateBudget(id, data)` y `deleteBudget(id)`.

*   **[] Tarea 6.2: Crear Página de Gestión de Presupuestos.**
    *   [] Crear la ruta y el archivo de página en `src/app/finance/budgets/page.tsx`.
    *   [] Servirá como el centro para que el usuario vea y administre todos sus presupuestos.

*   **[] Tarea 6.3: Componente `BudgetForm`.**
    *   [] Crear un componente reutilizable en `src/finance/components/BudgetForm.tsx`.
    *   [] El formulario debe incluir campos para `name`, `amount`, `month`, `year` y un selector (`<select>`) para `categoryId`. El selector se poblará con las categorías de tipo "EGRESO" obtenidas de `getCategories`.

*   **[] Tarea 6.4: Componente `BudgetList`.**
    *   [] Crear un componente en `src/finance/components/BudgetList.tsx`.
    *   [] Mostrará una lista de los presupuestos del mes/año seleccionado, permitiendo editar (abrir `BudgetForm` con datos precargados) y eliminar cada uno.

*   **[] Tarea 6.5: Integración y Lógica en la Página.**
    *   [] En `budgets/page.tsx`, integrar `BudgetForm` y `BudgetList`.
    *   [] Añadir selectores para filtrar por `año` y `mes`.
    *   [] Implementar la lógica para obtener y mostrar los presupuestos según el filtro y manejar las operaciones CRUD.

*   **[] Tarea 6.6: Añadir Enlace de Navegación.**
    *   [] En el layout principal del módulo de finanzas (ej. `src/app/finance/layout.tsx`) o en el dashboard, añadir un enlace para navegar a la página `/finance/budgets`.

#### **[] Fase 7: Implementación del Resumen Mensual**

[] Esta fase se centrará en visualizar el estado financiero consolidado del usuario en el dashboard principal, conectando transacciones, categorías y presupuestos.

*   **[] Tarea 7.1: Extender `financeService` para el Resumen.**
    *   [] En `src/finance/services/financeService.ts`, añadir la función `getMonthlySummary(year, month)` que llamará al endpoint `GET /finance/summary/:year/:month`.

*   **[] Tarea 7.2: Componente Principal `MonthlySummary`.**
    *   [] Crear un componente contenedor en `src/finance/components/MonthlySummary.tsx`.
    *   [] Este componente recibirá los datos del resumen y orquestará la presentación de los sub-componentes.

*   **[] Tarea 7.3: Sub-componentes de Visualización.**
    *   [] **`DateSelector`:** Crear un pequeño componente (puede ser parte de `MonthlySummary`) con selectores para que el usuario elija el `año` y `mes` del resumen a consultar.
    *   [] **`SummaryTotals`:** Crear un componente que muestre los totales (`totalIncome`, `totalExpenses`, `balance`) en un formato claro y visual (ej. tarjetas de estadísticas).
    *   [] **`CategorySummaryList`:** Crear un componente que itere sobre el array `categorySummaries` y muestre para cada categoría: `categoryName`, `budgeted`, `spent` y `remaining`. Utilizar barras de progreso o colores para indicar visualmente el estado del gasto (ej. verde si está por debajo del presupuesto, rojo si lo ha excedido).

*   **[] Tarea 7.4: Integración en el Dashboard.**
    *   [] Modificar la página `src/app/finance/dashboard/page.tsx`.
    *   [] Implementar la lógica de estado (`useState`, `useEffect`) para manejar el `año` y `mes` seleccionados.
    *   [] Al cargar la página o cuando el usuario cambie la fecha, llamar a `getMonthlySummary` y pasar los datos al componente `MonthlySummary`.
    *   [] Renderizar el componente `MonthlySummary` en el dashboard, reemplazando o complementando la vista actual.