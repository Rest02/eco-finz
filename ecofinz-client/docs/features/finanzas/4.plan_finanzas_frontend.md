...
#### **[] Fase 9: Funcionalidad de Edición Completa (Completing CRUD)**

[] Esta fase implementará la capacidad de actualizar (PATCH) las entidades principales, completando el ciclo CRUD que estaba pendiente para cuentas, categorías y transacciones.

*   **[] Tarea 9.1: Servicio de Actualización.**
    *   [] Asegurar que `financeService.ts` tenga implementados los métodos `updateAccount`, `updateCategory` y `updateTransaction`.

*   **[] Tarea 9.2: Edición de Cuentas.**
    *   [] Modificar `AccountList` para incluir un botón "Editar".
    *   [] Adaptar `AccountForm` para aceptar datos iniciales (modo edición).
    *   [] En el dashboard, implementar la lógica para cargar los datos en el formulario y enviar la actualización.

*   **[] Tarea 9.3: Edición de Categorías.**
    *   [] Modificar `CategoryList` para incluir un botón "Editar".
    *   [] Adaptar `CategoryForm` para modo edición.
    *   [] Implementar la lógica de actualización en `categories/page.tsx`.

*   **[] Tarea 9.4: Edición de Transacciones.**
    *   [] Modificar `TransactionList` para permitir acciones de edición en cada fila.
    *   [] Reutilizar `TransactionForm` para permitir la edición, precargando fecha, monto, descripción, cuenta y categoría.
    *   [] Implementar la lógica en la página de detalle de cuenta (donde se listan las transacciones).
