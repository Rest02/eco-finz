...
[] Esta fase se centrará en construir la interfaz y la lógica para que el usuario pueda administrar sus propias categorías de ingresos y egresos.

*   **[] Tarea 5.1: Creación de la Página de Categorías.**
    *   [] Crear la ruta y el archivo de página en `src/app/finance/categories/page.tsx`.
    *   [] Esta página será el centro de la gestión de categorías.

*   **[] Tarea 5.2: Componente `CategoryForm`.**
    *   [] Crear un nuevo componente reutilizable en `src/finance/components/CategoryForm.tsx`.
    *   [] Este formulario permitirá al usuario crear nuevas categorías (`{ "name": "...", "type": "..." }`).

*   **[] Tarea 5.3: Componente `CategoryList`.**
    *   [] Crear un nuevo componente reutilizable en `src/finance/components/CategoryList.tsx`.
    *   [] Mostrará una lista de las categorías existentes.
    *   [] Cada elemento de la lista incluirá un botón para "Eliminar".

*   **[] Tarea 5.4: Implementación de Funcionalidades CRUD.**
    *   [] En la página `categories/page.tsx`, integrar los componentes `CategoryForm` y `CategoryList`.
    *   [] Utilizar las funciones `getCategories`, `createCategory`, y `deleteCategory` del `financeService` para implementar la lógica.

*   **[] Tarea 5.5: Enlace a la Gestión de Categorías.**
    *   [] Añadir un enlace en la página de `dashboard` para que el usuario pueda navegar fácilmente a la nueva página de gestión de categorías.