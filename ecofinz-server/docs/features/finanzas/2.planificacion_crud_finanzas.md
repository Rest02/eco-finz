...
-   [x] Tarea 1: Generar el recurso para `Transaction`.
-   [x] Tarea 2: Implementar `CreateTransactionDto` y `UpdateTransactionDto`. El DTO de creación debe incluir `amount`, `type`, `date`, `accountId` y `categoryId`.
-   [x] Tarea 3: Implementar `createTransaction` (`POST /transactions`). La lógica debe verificar que `accountId` y `categoryId` pertenezcan al usuario.
-   [x] Tarea 4: Implementar `findAllTransactions` (`GET /transactions`) con soporte para filtros (rango de fechas, cuenta, categoría) y paginación.
-   [x] Tarea 5: Implementar `findOneTransaction` (`GET /transactions/:id`).
-   [x] Tarea 6: Implementar `updateTransaction` (`PATCH /transactions/:id`).
-   [x] Tarea 7: Implementar `deleteTransaction` (`DELETE /transactions/:id`).
-   [x] Tarea 8: Asegurar que toda la lógica de servicio valide que el usuario solo puede afectar sus propias transacciones.

---

### **Fase 4: CRUD para `Budget` (Presupuestos)**

-   [x] Tarea 1: Generar el recurso para `Budget`.
-   [x] Tarea 2: Implementar `CreateBudgetDto` y `UpdateBudgetDto`. El DTO necesita `amount`, `month`, `year` y `categoryId`.
-   [x] Tarea 3: Implementar `createBudget` (`POST /budgets`), validando que la categoría pertenece al usuario.
-   [x] Tarea 4: Implementar `findAllBudgets` (`GET /budgets`), permitiendo filtrar por mes y año.
-   [x] Tarea 5: Implementar `updateBudget` (`PATCH /budgets/:id`).
-   [x] Tarea 6: Implementar `deleteBudget` (`DELETE /budgets/:id`).
-   [x] Tarea 7: Proteger todas las rutas y validar la propiedad del usuario en la lógica de servicio.

---

### **Fase 5: Lógica para `MonthlySummary` (Resumen Mensual)**

-   [x] Tarea 1: Crear un endpoint de solo lectura `GET /summary/:year/:month` en el `FinanceController`.
-   [x] Tarea 2: Implementar en `FinanceService` un método que calcule dinámicamente el resumen del mes consultado, obteniendo transacciones y presupuestos del usuario.
-   [x] Tarea 3: La lógica debe calcular: total de ingresos, total de egresos, balance y gasto por categoría versus su presupuesto.
-   [x] Tarea 4: Devolver el objeto de resumen calculado en la respuesta del endpoint.
-   [x] Tarea 5: Proteger el endpoint con `JwtAuthGuard`.
