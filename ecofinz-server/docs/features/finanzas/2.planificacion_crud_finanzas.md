...
-   [] Tarea 1: Crear el módulo principal `FinanceModule` que contendrá toda la lógica.
-   [] Tarea 2: Generar los archivos base `finance.service.ts` y `finance.controller.ts` dentro del nuevo módulo.
-   [] Tarea 3: Importar `PrismaModule` en `FinanceModule` para proveer acceso a la base de datos a los servicios.
-   [] Tarea 4: Importar el nuevo `FinanceModule` en el `AppModule` principal para integrarlo a la aplicación.

---

### **Fase 1: CRUD para `Category` (Categorías)**

-   [] Tarea 1: Generar el recurso para `Category` usando la CLI de NestJS (`controller`, `service`, y `dto`).
-   [] Tarea 2: Implementar el DTO `CreateCategoryDto` y `UpdateCategoryDto` con las validaciones correspondientes.
-   [] Tarea 3: Implementar el método `createCategory` en el servicio y su endpoint `POST /categories`, asociando la categoría al usuario autenticado.
-   [] Tarea 4: Implementar el método `findAllCategories` para obtener solo las categorías del usuario autenticado (`GET /categories`).
-   [] Tarea 5: Implementar el método `updateCategory` y su endpoint (`PATCH /categories/:id`).
-   [] Tarea 6: Implementar el método `deleteCategory` y su endpoint (`DELETE /categories/:id`).
-   [] Tarea 7: Aplicar el guard `JwtAuthGuard` a todas las rutas del `CategoryController`.

---

### **Fase 2: CRUD para `Account` (Cuentas)**

-   [] Tarea 1: Generar el recurso para `Account` (controller, service, y DTOs).
-   [] Tarea 2: Implementar `CreateAccountDto` y `UpdateAccountDto` con sus validaciones.
-   [] Tarea 3: Implementar el método `createAccount` (`POST /accounts`), asociando la cuenta al usuario autenticado.
-   [] Tarea 4: Implementar `findAllAccounts` para obtener las cuentas del usuario (`GET /accounts`).
-   [] Tarea 5: Implementar `updateAccount` (`PATCH /accounts/:id`).
-   [] Tarea 6: Implementar `deleteAccount` (`DELETE /accounts/:id`).
-   [] Tarea 7: Proteger todas las rutas del `AccountController` con `JwtAuthGuard`.
